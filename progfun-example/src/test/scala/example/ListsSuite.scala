...
class ListsSuite extends FunSuite {
  ...
  val emp = List()
  val neg = List(-1, -3, -2)
  val zer = List(0, 0, 0, 0)
  val rep = List(1, 1, 1, 1, 1)
  ...
  test("sum of neg list"){
    assert(sum(neg) === -6)
  }

  test("max of neg list"){
    assert(max(neg) === -1)
  }
  
  test("sum of zero list"){
    assert(sum(zer) === 0)
  }
  
  test("max of zero list"){
    assert(max(zer) === 0)
  }
  
  test("sum of list wirh only 1s"){
    assert(sum(rep) === 5)
  }
  
  test("max of list with only 1s"){
    assert(max(rep) === 1)
  }
}
