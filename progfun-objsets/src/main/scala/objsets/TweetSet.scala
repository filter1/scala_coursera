...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def descendingByRetweet: TweetList = descByAcc(this)
    
  def descByAcc(set: TweetSet): TweetList =
    if (set.isEmpty) Nil
    else
    {
	    val mr = set.mostRetweeted
	  	new Cons(mr, descByAcc(set.remove(mr)))
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    lazy val fi = filter(tweet => tweet.retweets > elem.retweets)
    if (fi.isEmpty) elem
    else fi.mostRetweeted        
   }
...
object GoogleVsApple {
  ...
  lazy val all = TweetReader.allTweets.descendingByRetweet
